...
CREATE TABLE cloudtechdb.task4_top10tb1 AS SELECT SUM(Score) AS Grand_Post_Score, OwnerUserId AS Owner FROM cloudtechdb.top2gpoststb GROUP BY  OwnerUserId ORDER BY Grand_Post_Score DESC LIMIT 10;
 
--To store the OwnerUserId and Body from the previous table.
CREATE TABLE cloudtechdb.task4_top10tb2 AS SELECT OwnerUserId, Body FROM cloudtechdb.top2gpoststb A WHERE EXISTS (SELECT 1 FROM cloudtechdb.task4_top10tb1 B WHERE A.OwnerUserId=B.Owner);
 
--To remove the stop words
CREATE OR REPLACE VIEW cloudtechdb.task4_top10_vw AS SELECT OwnerUserId, de_word FROM cloudtechdb.task4_top10tb2 LATERAL VIEW explode(split(Body, ' ')) t AS de_word WHERE NOT is_stopword(de_word);
...
--TF-IDF
CREATE OR REPLACE VIEW cloudtechdb.task4_tfidf_vw AS SELECT tf.OwnerUserId, tf.de_word, tfidf(tf.freq, df.docs, ${n_docs}) AS wf_tfidf FROM cloudtechdb.task4_termfreq_vw tf JOIN cloudtechdb.task4_docfreq_vw df ON (tf.de_word = df.de_word) ORDER BY wf_tfidf DESC;
